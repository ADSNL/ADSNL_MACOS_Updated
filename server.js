...
app.get('/api/books', (req, res) => {
  ...
  conn.connect(function (err) {
    ...
    req.query(`SELECT TOP 4 Books.Book_ID,Books.Book_Title as Title,Books.ISBN_10 as Number,ISBN_13,Book_Genre_ID,Book_Publisher_ID,min(Unit_Price) as Price 
                  FROM  Books JOIN dbo.Book_Media_Lookup ON Books.Book_ID=Book_Media_Lookup.Book_ID 
                  GROUP BY Books.Book_ID,Book_Title,ISBN_10,ISBN_13,Book_Genre_ID,Book_Publisher_ID`, (err, recordset) => {
      if (err) {
        console.log(err);
        return;
      } else {
...
app.get('/api/customer', (req, res) => {
  ...
  conn.connect(function (err) {
    ...
    Street_Number as Street, Street_Type as Avenue from Customer_Master where Customer_FName like '%` + first_name + `%'`, (err, recordset) => {
      ...
      req.query(`select top 10 Customer_ID as ID, Customer_FName as FName, Customer_LName as LName, 
                  Sex as Gender, Birth_Date as DOB, Zip_Code as Zip, City, State, Street_Name as StreetName, 
                  Street_Number as Street, Street_Type as Avenue from Customer_Master 
                  where Customer_FName like '%` + first_name + `%' and Customer_LName like '%` + last_name + `%'`, (err, recordset) => {
        if (err) {
          console.log(err);
          return;
        } else {
...
app.get('/api/viewBooks', (req, res) => {
  ...
  conn.connect(function (err) {
    ...
    FROM (
        SELECT Books.Book_ID,Book_Title as Title,ISBN_10 as Number,ISBN_13,Book_Genre_ID,Book_Publisher_ID,min(Unit_Price) as Price, 
        ROW_NUMBER() OVER (ORDER BY Books.Book_ID) AS RowNum
        FROM  Books JOIN dbo.Book_Media_Lookup ON Books.Book_ID=Book_Media_Lookup.Book_ID GROUP BY Books.Book_ID,
        Book_Title,ISBN_10,ISBN_13,Book_Genre_ID,Book_Publisher_ID
    ) AS BD
...
app.get('/api/viewClothing', (req, res) => {
  ...
  conn.connect(function (err) {
    ...
    FROM (
        SELECT Clothing_ID as Number, Clothing_Name as Title, Price, (SElECT Count(*) FROM Clothing) AS CatCount, 
        ROW_NUMBER() OVER (ORDER BY Clothing.Clothing_ID) AS RowNum
        FROM Clothing
    ) AS C
...
app.get('/api/viewKitchen', (req, res) => {
  ...
  conn.connect(function (err) {
    ...
    FROM (
        SELECT K.Kitchen_Product_ID as Number, K.Kitchen_Product_Name as Title, KT.Price as Price, (SElECT Count(DISTINCT K.Kitchen_Product_ID) 
        FROM Kitchen AS K JOIN Kitchen_Types_Lookup AS KT ON K.Kitchen_Product_ID = KT.Kitchen_Product_ID) AS CatCount, 
        ROW_NUMBER() OVER (ORDER BY k.Kitchen_Product_ID) AS RowNum
...
app.get('/api/search/', (req, res) => {
  ...
  conn.connect(function (err) {
    ...
    FROM (
        SELECT Books.Book_ID,Book_Title as Title,ISBN_10 as Number,ISBN_13,Book_Genre_ID,Book_Publisher_ID,min(Unit_Price) as Price, 
        ROW_NUMBER() OVER (ORDER BY Books.Book_ID) AS RowNum
        FROM  Books JOIN dbo.Book_Media_Lookup ON Books.Book_ID=Book_Media_Lookup.Book_ID GROUP BY Books.Book_ID,Book_Title,
        ISBN_10,ISBN_13,Book_Genre_ID,Book_Publisher_ID
    ) AS BD
    ...
    if (category == "Kitchen") {
      req.query(`SELECT K.Number, K.Title, K.Price, K.CatCount
      FROM (
          SELECT K.Kitchen_Product_ID as Number, K.Kitchen_Product_Name as Title, KT.Price as Price, 
          (SElECT Count(DISTINCT K.Kitchen_Product_ID) FROM Kitchen AS K JOIN Kitchen_Types_Lookup AS KT ON K.Kitchen_Product_ID = KT.Kitchen_Product_ID) AS CatCount, 
          ROW_NUMBER() OVER (ORDER BY k.Kitchen_Product_ID) AS RowNum
          FROM Kitchen AS K JOIN Kitchen_Types_Lookup AS KT ON K.Kitchen_Product_ID = KT.Kitchen_Product_ID
      ) AS K
    ...
    if (category == "Make up") {
      req.query(`SELECT M.Number, M.Title, M.Price, M.CatCount
      FROM (
          SELECT Makeup_ID as Number, Makeup_Name as Title, 29 as Price, (SElECT Count(*) FROM Makeup) AS CatCount, 
          ROW_NUMBER() OVER (ORDER BY Makeup_ID) AS RowNum
          FROM Makeup
      ) AS M
    WHERE M.Title like '%` + parameter + `%'`, (err, recordset) => {
        if (err) {
